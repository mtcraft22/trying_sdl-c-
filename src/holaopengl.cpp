...
int main(int argc, char *argv[]) {
    ...
    SDL_Texture *awita =
            SDL_CreateTextureFromSurface(renderizador, texture_awita);
    ...
    SDL_Texture *capa2_text =
            SDL_CreateTextureFromSurface(renderizador, segunda_capa);
    if (!tex) {
        cout << "sdl_texture_load" << SDL_GetError() << std::endl;
    }

    SDL_Color col{0xff, 0xff, 0xff, 0xff};
    SDL_Color col2{0xff, 0xff, 0xff, 0xff};

    Chunck *trozo = new Chunck(75, 15, 15, 550, 120, &col);
    Chunck *mar = new Chunck(75, 15, 15, 550, 140, &col2);
    Chunck *piedra = new Chunck(75, 15, 15, 550, 100, &col2);

    SDL_Event e;
    bool quit = false;
    SDL_Vertex pepe_forma[3] = {{200, 200, col, 0.0f, 0.0f},
                                {250, 200, col, 1.0f, 0.0f},
                                {200, 250, col, 0.0f, 1.0f}};
    SDL_Vertex pepe_forma2[3] = {{200, 250, col, 0.0f, 1.0f},
    ...
    vector<Chunck *> layers = {piedra, trozo, mar};
    while (!quit) {
        ...
        if (delta > sdl_ticks_4_frame) {
            ...
            trozo->draw_chunk(renderizador, capa2_text);
            piedra->draw_chunk(renderizador, tex);
            ...
            while (SDL_PollEvent(&e)) {
                if (e.type == SDL_QUIT) {
                    ...
                    switch (e.key.keysym.sym) {
                        case SDLK_d:
                            angle--;
                            if (seleciono->point_hit(bolita.getcenterx(),
                                                     bolita.getcentery())) {
                ...
                for (int i = 0; i < layers.at(0)->gridgroup.size(); i++) {
                    for (int j = 0; j < layers.at(0)->gridgroup.at(i).size(); j++) {
                        terreno *ter = layers.at(0)->gridgroup.at(i).at(j);
                        terreno *ter2 = layers.at(1)->gridgroup.at(i).at(j);
                        terreno *ter3 = layers.at(2)->gridgroup.at(i).at(j);
                        terreno *terlay = ter;
                        ...
                        ter2->poligono1[0].color = col;
                        ter2->poligono1[1].color = col;
                        ter2->poligono1[2].color = col;
                        ter2->poligono2[0].color = col;
                        ter2->poligono2[1].color = col;
                        ter2->poligono2[2].color = col;
                        ...
                        if (ter2->poligono1[0].position.y > ter3->poligono1[0].position.y) {
                            ter2->poligono1[0].color = SDL_Color{0, 0, 0, 0};


                        }
                        if (ter2->poligono1[1].position.y > ter3->poligono1[1].position.y) {
                            ter2->poligono1[1].color = SDL_Color{0, 0, 0, 0};

                        }
                        if (ter2->poligono1[2].position.y > ter3->poligono1[2].position.y) {
                            ter2->poligono1[2].color = SDL_Color{0, 0, 0, 0};

                        }
                        if (ter2->poligono2[0].position.y > ter3->poligono2[0].position.y) {
                            ter2->poligono2[0].color = SDL_Color{0, 0, 0, 0};

                        }
                        if (ter2->poligono2[1].position.y > ter3->poligono2[1].position.y) {
                            ter2->poligono2[1].color = SDL_Color{0, 0, 0, 0};

                        }
                        if (ter2->poligono2[2].position.y > ter3->poligono2[2].position.y) {
                            ter2->poligono2[2].color = SDL_Color{0, 0, 0, 0};

                        }
                        if (
                                (ter->poligono2[2].position.y > ter2->poligono2[2].position.y)
                                &&
                                (ter->poligono2[1].position.y > ter2->poligono2[1].position.y)
                                &&
                                (ter->poligono2[0].position.y > ter2->poligono2[0].position.y)
                                &&
                                (ter->poligono1[2].position.y > ter2->poligono1[2].position.y)
                                &&
                                (ter->poligono1[1].position.y > ter2->poligono1[1].position.y)
                                &&
                                (ter->poligono1[0].position.y > ter2->poligono1[0].position.y)
                            )
                        {
                            terlay = ter2;
                        }else{
                        ...
                        if (terlay->point_hit(mx, my)) {
                            terlay->poligono1[0].color = SDL_Color{0, 100, 0, 255};
                            terlay->poligono1[1].color = SDL_Color{0, 100, 0, 255};
                            terlay->poligono1[2].color = SDL_Color{0, 100, 0, 255};
                            terlay->poligono2[0].color = SDL_Color{0, 100, 0, 255};
                            terlay->poligono2[1].color = SDL_Color{0, 100, 0, 255};
                            terlay->poligono2[2].color = SDL_Color{0, 100, 0, 255};
                            seleciono = terlay;
                        }
                    }
                }
